...
+ static修饰的变量存储在全局区，如没有显示初始化则自动初始化为0，生命周期是直到程序运行结束才被释放；
+ static修饰的全局变量在程序运行一开始完成初始化，只能作用于本文件，而非静态全局变量作用于整个工程文件，只要在其他文件用`extern <全局变量名>`就可以使用；
+ static修饰的局部变量只在第一次执行到此处的时候进行初始化
+ static修饰的函数只能在本文件中调用
* 
+ static修饰成员变量和成员函数属于整个类，所有对象共享，在实例化对象之前就已经分配空间
+ static修饰的成员变量必须类内声明，类外初始化
+ static修饰的成员函数只能使用静态成员变量，和静态成员函数
...
+ 右值：在赋值号右边取出值赋给其他变量的值，可以在内存中，也可以在CPU寄存器中
**  一个对象被用作右值时，使用的是它的内容(值)，被当作左值时，使用的是它的地址**
+ 左值引用：type &引用名 = 左值表达式；（常用）
+ 右值引用：type &&引用名 = 右值表达式；
...
#### 6.1.8 友元
+ 全局函数做友元，在类中声明函数并用`friend`修饰（`friend void fun();`）
+ 类做友元，在类中声明类并用`friend`修饰(`friend class A;`)
...
#### 6.3.3 多态内部原理（虚函数表）
+ 每个有虚函数的类，编译器都会为其生成一张虚函数表（存放每个虚函数的函数地址，按函数声明顺序排列），所有实例对象共享虚表，在对象实例化时，最前面存放一个虚函数表指针，指向虚表地址，也就是说编译时完成虚表创建，程序运行完成虚表指针的赋值。
+ 当子类继承父类，并重写父类虚函数时，它的虚表形式为：重写的虚函数地址替换父类对应的虚函数地址，其他不变，后面跟着自己的虚函数地址。
+ 如果是多继承，会生成多个虚函数表（有虚表的父类），每个虚表结构和上面单继承一致，当对象实例化时也会生成多个虚函数表指针。
...
+ select：无差别轮询所有流，找出能读出数据，或者写入数据的流，对他们进行操作。有最大连接数限制，32位机为1024，64位机为2048.
+ poll：和select类似，都是通过轮询的方式，区别是他内部通过链表实现的，所以没有连接数限制
+ epoll：（Linux特有）基于事件驱动，没有最大并发限制，效率提升，不是轮询的方式，不会随着FD数目的增加效率下降。只有活跃可用的FD才会调用callback函数。有LT模式和ET模式。

+ select 使用
`int select(int maxfdp,fd_set *readfds,fd_set *writefds,fd_set *errorfds,struct timeval *timeout);`
> 返回值：负值：select错误；正值：某些文件可读写或出错；0：等待超时，没有可读写或错误的文件
> maxfdp:所有文件描述符的最大值加1;
> readfds:指向fd_set结构的指针，这个集合中应该包括文件描述符，我们是要监视这些文件描述符的读变化的
> writefds:同上，用来监视可写变化
> errorfds:同上，用来监视文件错误
> timeout：时间结构体：如传入NULL，则阻塞，一直到某个文件状态改变；传0，立即返回，有变化返回正值，无变化返回0；具体秒数：则等待监视，超时返回0；


## 其他
### 关键字
+ violate：修饰的变量不会使用编译器优化，每次都会从内存中读取，不会使用寄存器的值
+ explict：用于类内的单参数（或多参数，但至少除第一个后面的都有默认值）构造函数前面，声明为显示构造（默认的都为隐式构造），阻止不合适的构造对象。
+ mutalbe：用于修饰成员变量，使在const成员函数中可以修改
+ extern：使变量或函数可以在其他文件中使用，1.在头文件中用extern声明，其他文件包含头文件2.在其他文件中直接extern
+ extern “C”{}：让编译器按照c风格编译函数，因为c++要解决多态问题，函数重载，会将函数名编译成一个中间函数名。
...
+ []：变量捕获，值捕获和引用捕获
+ ()：操作符重载函数参数
+ Type:返回值类型，如果为空，可以省略
+ {}：函数体
+ eg:`[] (int x, int y) -> int { int z = x + y; return z; }`
+ eg:`[=](this){cout << "hello" << endl;}`